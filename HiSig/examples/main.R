...
parser$add_argument("--s", required=T, help="the system list")
...
terms = readLines(args$s)
data <- load_data(args$x, args$y, genes, terms, index1 = F, gene.as.term=args$g_as_s)

hisig_out <- hisig_fit(data, nlambda=args$nlambda, pos.only = args$pos_only)
beta_max_rand <- hisig_fit_rand(data, hisig_out$lambda, batch=args$permute/10, pos.only = args$pos_only)

impact <- cbind(hisig_out$beta.max, beta_max_rand)

df <- parse_hisig(data, impact, term.names = terms, gene.names = genes, gene.as.term=args$g_as_s)
